...
pub enum StreamServerMessage {
    ...
    /// The current native ICE allocation cannot produce a usable UDP path.
    /// The browser should replace the whole session instead of waiting for the
    /// failed peer's normal timeout.
    RetryFreshSession {
        reason: String,
    },
    PeerDisconnect,
